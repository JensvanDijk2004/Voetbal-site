<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8 mt-2">
        <span class="text-gray-800 font-bold text-2xl">Wedstrijd uitslag</span>
        <div class="flex items-center justify-between mb-6 mt-4">
            ...
            <div class="flex flex-col items-center">
                ...
                <div class="font-bold text-gray-800 text-2xl">{{ $game->home_score }} - {{ $game->away_score }}</div>
        ...
        <div class="mt-8">
            <h2 class="text-gray-800 font-bold text-xl mb-4">Doelpunten</h2>
            <ul>
                @foreach($game->goals as $goal)
                    <li>
                        @if($goal !== null)
                            {{ $goal->user->name }} ({{ $goal->user->team->name }}) - {{ $goal->minute. "'s minuut" }}
                        @endif
                    </li>
                @endforeach
            </ul>
